...
Julius.ContentLibrary.Migrate.itemViewModel = function (moduleId, resx) {
    var service = {
    ...
    var loadUsers = function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            var result = data[i];
            var user = new Julius.ContentLibrary.Migrate.user(result.id, result.name);
...
Julius.ContentLibrary.Migrate.user = function (id, name) {
    this.id = id;
    this.name = name;
}